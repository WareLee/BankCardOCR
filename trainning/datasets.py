...
def concat(img,img_name,count,annotation,img_list):
    ...
    H,W,C = img.shape
    num = int(rand(4,6))  # 横向连接多少个号码片段
    imgs = random.sample(img_list,num)  # TODO 这里是随机采样，是否可以用穷举法组合
    dst = np.zeros((H,W*(num+1),C),np.uint8) # 新拼接出来的长条图片
    for h in range(H):
        for w in range(W):
            dst[h,w] = img[h,w]
